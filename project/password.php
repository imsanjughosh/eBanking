...
if(isset($_SESSION['ardent']['id'])){
	...
	$verify = mysqli_query($connect,"SELECT id FROM open WHERE id='$id'");
...
<html>
	...
	$query= "select * from open WHERE password='$password' and id='$id'";
	...
	$count=mysqli_num_rows($execute);
			...
			{
				$query = mysqli_query($connect,"UPDATE open SET password='$confirmpassword' WHERE id='$id'") or die(mysqli_error($connect));
				if($query){
		header("Location:password.php?msg= Password Update Success");
	                      }
			}
			else
			{
				header("Location:password.php?msg= Password Update Fail");
			}
			}
			else{
